...
class ProjE:
    ...
    def __init__(self, data_dir, embed_dim=100):
        ...
        with open(os.path.join(data_dir, 'entity2id.txt'), 'r') as f:
            self.__entity_id_map = {x.strip().split('\t')[0]: int(x.strip().split('\t')[1]) for x in f.readlines()[1:]}
        ...
        with open(os.path.join(data_dir, 'relation2id.txt'), 'r', encoding='utf-8') as f:
            self.__relation_id_map = {x.strip().split('\t')[0]: int(x.strip().split('\t')[1]) for x in f.readlines()[1:]}
        ...
        def load_triple(file_path):
            with open(file_path, 'r', encoding='utf-8') as f_triple:
                return np.asarray([[self.__entity_id_map[x.strip().split('\t')[0]],
                                    ...
                                    self.__relation_id_map[x.strip().split('\t')[2]]] for x in f_triple.readlines()[1:]],
                                  dtype=np.int32)
        ...
        def gen_tr_h(triple_data):
            ...
            for h, t, r in triple_data:
                ...
                tr_h[t][r].add(h)
            return tr_h
        def load_triple2(file):
            with open(file,'r') as f:
                return np.asarray([[x.strip().split()[0],
                                    x.strip().split()[1],
                                    x.strip().split()[2]] for x in f.readlines()[1:]],dtype=np.int32) # 因为原本的数据已经变化所以不用重新制作train2id.txt
        self.__train_triple = load_triple2(os.path.join(data_dir, 'train2id.txt'))
        print("N_TRAIN_TRIPLES: %d" % self.__train_triple.shape[0])

        self.__test_triple = load_triple2(os.path.join(data_dir, 'test2id.txt'))
        print("N_TEST_TRIPLES: %d" % self.__test_triple.shape[0])

        self.__valid_triple = load_triple2(os.path.join(data_dir, 'valid2id.txt'))
...
def main(_):
    ...
    parser.add_argument('--data', dest='data_dir', type=str, help="Data folder", default='./benchmarks/FB15k/')
